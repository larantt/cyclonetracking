...
verd = f"{argv[10]}" # Detection Version
vert = f"{argv[11]}" # Tracking Version

path = "/home/laratt/Documents/cycloneTracking/scriptTracking"
inpath = path+verd+vert
outpath = inpath
...
*******************************************'''
# File Variables
typ = f'{argv[15]}' # Cyclone, System, or Active

bboxName = path+"/Projections/EASE2_N0_25km_Projection.nc"
bbox = [int(argv[18]),int(argv[19]),int(argv[20]),int(argv[21])] # minlat, maxlat, minlon, maxlon
bboxnum = f'{argv[14]}'
bboxmain = "" # The main bbox your subsetting from; usually "" for "all cyclones", otherwise BBox##

# Time Variables
starttime = [int(argv[3]),int(argv[5]),int(argv[7]),0,0,0] # Format: [Y,M,D,H,M,S]
endtime = [int(argv[4]),int(argv[6]),int(argv[8]),0,0,0] # stop BEFORE this time (exclusive)
...
minlifespan = int(argv[16]) # in days
mintracklength = int(argv[17]) # in km for version 11+, in gridcells for version 10
